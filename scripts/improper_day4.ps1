...
Foreach ($List in $1){
    ...
    if (($Email.Split("@")[0] -ne ($FName+"_"+$LName)) -and (($Country -eq "Australia") -or ($Country -eq "UK")) -and ((($Email.Split("@")[1].Split(".")[0] -eq "gmail")) -or ($Email.Split("@")[1].Split(".")[0] -eq "hotmail") -or ($Email.Split("@")[1].Split(".")[0] -eq "yahoo"))){
